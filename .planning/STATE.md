...
Versión: v10.76.5 | Hito: BOT-BUILD-TOOLCALL-FB1-084 — Post-proceso de args Rama A Qwen; G0-TOOLCALL VERDE 10/10; G0-EXTRACTION VERDE 20/20; F3 listo para dictamen de flip del Auditor | Coherence Score: 1.000 (913 recolectados físico = 908 tests/ + 5 scripts/; 913/913 PASSED; 0 failed; 0 skipped)

### Current Position
**Phase:** BOT-BUILD-TOOLCALL-FB1-084 (v10.76.5) — implementación del fallback FB-1 aprobado en BOT-PLAN-TOOLCALL-CASE9-083: post-proceso de args en el adapter `DualProviderClient` que poda propiedades opcionales inferidas fuera de un léxico declarativo, sin tocar prompts/personality ni ai_brain.py. Commit A: `app/services/llm_client_service.py` — hook `_prune_ungrounded_args` post-parse de Rama A nativa, validador léxico para `calculate_credit_score.ocupacion`/`ocupacion_y_contrato`, log `[ARG-PRUNE]` ZSF sin valor crudo; cero pruning en Rama B, Gemini y propiedades requeridas; tests T29-T31. Commit B: re-ejecución de G0-TOOLCALL con `qwen-turbo` + FB-1 (10 casos, criterios idénticos) y G0-EXTRACTION de no-regresión. Resultados: G0-TOOLCALL VERDE (10/10: casos 7 y 10 divergen en Rama A pero coinciden en Rama B), G0-EXTRACTION VERDE (20/20). C4 intacto; doc `llm_runtime/global.qwen_enabled=false` intacto. Eval 913/913 Score 1.000.
**Status:** FB-1 IMPLEMENTADO — G0-TOOLCALL CERRADO; F3 CANDIDATO A FLIP PERO PENDIENTE DE DICTAMEN DEL AUDITOR (v10.76.5):
  - **Objetivo:** cerrar el caso 9 de G0-TOOLCALL (y el hazard C5-114) mediante post-proceso de args en el adapter, manteniendo criterios de gate sin relajar y sin adopción de modelo superior (probe qwen-plus/qwen-max agotado en 083).
  - **Implementación:** Commit A — `_prune_ungrounded_args` en `llm_client_service.py` + T29-T31. Commit B — G0-TOOLCALL con `qwen-turbo` + FB-1 y G0-EXTRACTION de no-regresión; reporte `scripts/gates_f4/g0_toolcall_report_fb1.json`.
  - **Pins:** T29 (`ocupacion="trabajo"` opcional fuera de léxico → podada); T30 (`ocupacion="independiente"` dentro de léxico → preservada); T31 (propiedad requerida fuera de léxico → NO podada; Rama B sin pruning).
  - **Eval:** Score 1.000 ≥ 0.9 — DEPLOY AUTHORIZED ✅. 913/913 PASSED; collect-only 913 (908 tests/ + 5 scripts/).
  - **Cambio acotado:** `app/services/llm_client_service.py` + `tests/test_llm_client_service_qwen_079.py` (T29-T31) + `scripts/gates_f4/g0_toolcall_report_fb1.json` + C6 docs. C4 intacto: `app/core/prompts.py`, `app/core/personality.json`, `admin.py`, `config_loader.py`, `ai_brain.py`, workflows sin toques; `audio_service.py` y `EXTRACTION_SCHEMA` sin modificar.
  - **Evidencia gates:** G0-TOOLCALL VERDE (10/10); G0-EXTRACTION VERDE (20/20). El camino F3 está técnicamente desbloqueado en la rama Qwen según los criterios F4 certificados; el flip productivo requiere dictamen explícito del Auditor y sign-off de Tobias.
