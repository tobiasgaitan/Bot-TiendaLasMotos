...
Versión: v10.76.1 | Hito: BOT-BUILD-MIGRATE-QWEN-079-F1.5 — Hotfix adapter dual-provider Qwen/Gemini (F1.5 HOTFIX COMPLETE) bajo orden literal P2 + candado H3-1 | Coherence Score: 1.000 (901 recolectados físico = 896 tests/ + 5 scripts/; 901/901 PASSED; 0 failed; 0 skipped)

### Current Position
**Phase:** BOT-BUILD-MIGRATE-QWEN-079-F1.5 (v10.76.1) — hotfix del adapter dual-provider dormido. Fixes F1-F6 en `app/services/llm_client_service.py`: F1 corrige duplicación del turno actual en `send_message` y mapeo inválido de `function_response` a OpenAI; F2 fuerza `GEMINI_MODEL_ID` en las 3 patas de failover; F3 evita que `.text` de Rama B devuelva el envelope JSON crudo; F4 cachea el cliente Firestore del poll; F5 preserva mensajes `role=system` al recortar historial; F6 cablea `format_qwen_error_structured` en telemetría de failover/error. Tests T7 re-pin, T9 extensión, T18/T19/T20 aditivos (20 pines en `tests/test_llm_client_service_qwen_079.py`). C4 intacto: `app/core/prompts.py` y `app/core/personality.json` byte-idénticos; sin toques a `admin.py`/`config_loader.py`/workflows. Eval 901/901 Score 1.000.
**Status:** F1.5 HOTFIX COMPLETE (v10.76.1):
  - **Objetivo:** corregir 3 bugs críticos y 3 issues menores detectados en revisión externa F1 (F3 [APROBADO CON CONDICIONES]) antes de cualquier avance F2/F3.
  - **Implementación:** `app/services/llm_client_service.py` — F1 `_build_openai_messages` delega a historial convertido; `send_message` cuenta turno actual por delta. F2 helper `_gemini_model()`. F3 `_parse_openai_response` en Rama B concatena parts texto parseadas. F4 `_FLAG_DB_CLIENT` + `_get_flag_db_client()`. F5 `_trim_history_to_budget` evicciona `role != "system"`. F6 `forensic=` en `[DUAL FAILOVER]` + `[QWEN FORENSIC]` en no-retriables. `tests/test_llm_client_service_qwen_079.py` — T1-T20 (20 pines).
  - **Pins:** T18 (turno actual 1× + `function_response` → `role:"tool"`); T19 (failover usa `GEMINI_MODEL_ID` en sync/async/chat); T20 (Rama B `.text` limpio).
  - **Eval:** Score 1.000 ≥ 0.9 — DEPLOY AUTHORIZED ✅. 901/901 PASSED; collect-only 901 (896 tests/ + 5 scripts/).
  - **Cambio acotado:** solo `app/services/llm_client_service.py` + `tests/test_llm_client_service_qwen_079.py` + C6 docs. C4 intacto: `app/core/prompts.py` y `app/core/personality.json` byte-idénticos; sin toques a `admin.py`, `config_loader.py`, workflows.
  - **Colaterales:** C5-098 (`get_active_model_id` ignora `role`) y C5-099 (simplificación bindings `QWEN_TURBO_API_KEY`/`QWEN_BASE_URL`) registrados; no se arreglan en F1.5. Deploy beta bloqueado hasta que el owner cree `QWEN_OMNI_API_KEY`, `QWEN_TURBO_API_KEY`, `QWEN_BASE_URL` en Secret Manager (patrón 205).
