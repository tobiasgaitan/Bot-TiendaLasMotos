...
Versión: v10.76.3 | Hito: BOT-BUILD-QWEN-SPLIT-081 — Ruteo por rol en DualProviderClient; gates F4 re-ejecutados con rol correcto; F3 BLOQUEADO (G0-TOOLCALL ROJO en qwen-turbo, G0-EXTRACTION ROJO 19/20, G0-AUDIO-VISION ROJO audio inline 400); G0-CONTEXT VERDE | Coherence Score: 1.000 (906 recolectados físico = 901 tests/ + 5 scripts/; 906/906 PASSED; 0 failed; 0 skipped)

### Current Position
**Phase:** BOT-BUILD-QWEN-SPLIT-081 (v10.76.3) — implementación del ruteo por rol en el facade `DualProviderClient` y re-ejecución de los 4 gates F4 contra el modelo Qwen correcto según el rol. Commit A: `app/services/llm_client_service.py` — `get_active_model_id(role)`, `_qwen_model(role)`, `DualProviderClient(role)`, `DualProviderChat` hereda rol, clave de cache `f"{_client_key(...)}|role={role}"` con backend Gemini compartido; `ai_brain.py`/`judge_service.py` → `role="agentic"`; `vision_service.py`/`audio_service.py` → `role="multimodal"`; tests T21-T24. Commit B: `scripts/gates_f4/g0_toolcall.py`/`g0_extraction.py` con `role="agentic"`/`qwen-turbo`; `g0_context.py`/`g0_audio_vision.py` con `role="multimodal"`/`qwen-omni-turbo`; patch local de `is_qwen_enabled`, retry ante `ConnectError`/`ReadTimeout`, URL verbatim, baseline con flag real false. Resultados: G0-TOOLCALL ROJO (2/10 casos divergen de Gemini; Rama B también diverge), G0-EXTRACTION ROJO (19/20 fixtures verdes; caso 18 diverge), G0-CONTEXT VERDE (5-turn recall + max_output 2048), G0-AUDIO-VISION ROJO (imagen PNG verde; audio WAV 400 "URL does not appear to be valid"; combinado 400). C4 intacto; doc `llm_runtime/global.qwen_enabled=false` intacto. Eval 906/906 Score 1.000.
**Status:** SPLIT ROLE ROUTING IMPLEMENTADO — F3 BLOQUEADO POR EVIDENCIA DE GATE (v10.76.3):
  - **Objetivo:** diseñar e implementar la estrategia SPLIT sobre el facade existente: `ai_brain.py`/`judge_service.py` usan `qwen-turbo` (agentic tool-calling); `vision_service.py`/`audio_service.py` usan `qwen-omni-turbo` (multimodal), y re-ejecutar los gates F4 con el rol correcto antes del flip F3.
  - **Implementación:** Commit A — ruteo por rol en `llm_client_service.py` + adquisición por rol en 4 servicios + T21-T24. Commit B — 4 scripts de gate actualizados por rol con mecanismo (b) certificado. Reportes JSON en `scripts/gates_f4/g0_*_report.json`.
  - **Pins:** T21 (`get_active_model_id` honra rol, C5-098 cerrado); T22 (cache del facade por rol, backend Gemini compartido, payload model por rol); T23 (`DualProviderChat` hereda rol); T24 (failover DUAL usa `GEMINI_MODEL_ID` independientemente del rol).
  - **Eval:** Score 1.000 ≥ 0.9 — DEPLOY AUTHORIZED ✅. 906/906 PASSED; collect-only 906 (901 tests/ + 5 scripts/).
  - **Cambio acotado:** `app/services/llm_client_service.py` + 4 servicios + `tests/test_llm_client_service_qwen_079.py` (T21-T24) + `scripts/gates_f4/*` + C6 docs. C4 intacto: `app/core/prompts.py`, `app/core/personality.json`, `admin.py`, `config_loader.py`, workflows sin toques.
  - **Evidencia gates:** G0-TOOLCALL ROJO (10 casos, 2 divergencias Rama A, 2 Rama B); G0-EXTRACTION ROJO (20 fixtures, 1 divergencia); G0-CONTEXT VERDE (2/2); G0-AUDIO-VISION ROJO (3 casos: imagen VERDE, audio WAV y combinado ROJO con 400). F3 queda bloqueado hasta dictamen del Auditor.
