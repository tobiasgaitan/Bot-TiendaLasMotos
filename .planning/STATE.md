...
Versión: v10.76.4 | Hito: BOT-BUILD-SPLIT-HARDENING-082 — Hardening de transporte del adapter; EXTRACTION y AUDIO-VISION VERDES; TOOLCALL ROJO caso 9; F3 BLOQUEADO | Coherence Score: 1.000 (910 recolectados físico = 905 tests/ + 5 scripts/; 910/910 PASSED; 0 failed; 0 skipped)

### Current Position
**Phase:** BOT-BUILD-SPLIT-HARDENING-082 (v10.76.4) — hardening de transporte sobre el facade `DualProviderClient` para cerrar los 3 gates F4 rojos de 081 sin relajar criterios. Commit A: `app/services/llm_client_service.py` — annex `[TRANSPORT TOOLCALL RULES]` (3 reglas anti-inferencia) solo rama Qwen con tools; annex `[TRANSPORT FIELD RULES]` derivado por tipo del schema (STRING abierto anti-booleanización / BOOLEAN estricto); guard `_contents_have_audio` en `_generate_content_sync`/`_generate_content_async`/`DualProviderChat.send_message`: audio → path Gemini salvo `QWEN_AUDIO_ENABLED=true` (default false); `_send_via_gemini` usa `GEMINI_MODEL_ID`; tests T25-T28 (T4/T14 ajustados por los nuevos annexes). Commit B: re-ejecución de 3 gates F4 con mecanismo (b) certificado. Resultados: G0-EXTRACTION VERDE (20/20 fixtures, caso 18 `servicios_publicos` pasa), G0-AUDIO-VISION VERDE (imagen→Qwen omni; audio/combinado→guard fuerza Gemini), G0-TOOLCALL ROJO (caso 9: Rama A infiere `ocupacion="trabajo"` pese a regla anti-inferencia; Rama B emulada devuelve texto sin invocar tool; criterio certificado exige paridad → STOP). C4 intacto; doc `llm_runtime/global.qwen_enabled=false` intacto. Eval 910/910 Score 1.000.
**Status:** HARDENING IMPLEMENTADO — F3 BLOQUEADO POR G0-TOOLCALL CASO 9 (v10.76.4):
  - **Objetivo:** llevar los 3 gates F4 rojos de 081 a VERDE mediante hardening de transporte (sin tocar prompts/personality) y, si alguno permanece ROJO, entregar evidencia al Auditor sin flip F3.
  - **Implementación:** Commit A — 3 adiciones de transporte en `llm_client_service.py` + T25-T28 + ajustes T4/T14. Commit B — re-ejecución de G0-TOOLCALL (Rama A/B contra turbo), G0-EXTRACTION y G0-AUDIO-VISION con reportes JSON.
  - **Pins:** T25 (audio+flag on+env ausente → path Gemini, ningún POST a DashScope); T26 (env true → audio a Qwen; flag off → Gemini aunque env true); T27 (FIELD-RULES presente en Qwen con schema, ausente sin schema); T28 (TOOLCALL RULES presente solo en Qwen con tools).
  - **Eval:** Score 1.000 ≥ 0.9 — DEPLOY AUTHORIZED ✅. 910/910 PASSED; collect-only 910 (905 tests/ + 5 scripts/).
  - **Cambio acotado:** `app/services/llm_client_service.py` + `tests/test_llm_client_service_qwen_079.py` (T25-T28 + T4/T14) + `scripts/gates_f4/*_report.json` + C6 docs. C4 intacto: `app/core/prompts.py`, `app/core/personality.json`, `admin.py`, `config_loader.py`, workflows sin toques; `audio_service.py` y `EXTRACTION_SCHEMA` sin modificar.
  - **Evidencia gates:** G0-EXTRACTION VERDE (20/20); G0-AUDIO-VISION VERDE (3/3); G0-TOOLCALL ROJO (caso 9: ni Rama A ni Rama B coinciden con baseline). F3 permanece bloqueado hasta dictamen del Auditor; prohibido relajar criterios.
