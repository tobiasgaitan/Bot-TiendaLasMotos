...
- **Workflow Environment & Services:** Added setup steps for Python and `astral-sh/setup-uv@v5`, created virtualenv (`uv venv`), installed python dependencies, set up mock configuration variables in the workflow's job environment, and added a background startup step for the FastAPI app server (`uv run uvicorn app.main:app --port 8000 &`).
...
|------|--------|-------------|
| [.github/workflows/qa-pipeline.yml](file:///Users/tobiasgaitangallego/Bot-TiendaLasMotos/.github/workflows/qa-pipeline.yml) | Modified | Removed node cache, configured python, uv venv, and uvicorn startup. |
