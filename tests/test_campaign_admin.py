...
async def test_campaign_orchestrator_failure_isolation():
    ...
    doc_mock_1.to_dict.return_value = {"nombre": "Juan", "moto_interes": "Boxer CT"}
    doc_mock_1.id = "3001234567"
    doc_mock_1.reference.update = AsyncMock()

    doc_mock_2 = MagicMock()
    doc_mock_2.to_dict.return_value = {"nombre": "Maria", "moto_interes": "Pulsar NS"}
    doc_mock_2.id = "3009876543"
    doc_mock_2.reference.update = AsyncMock()
    
    # Mock de DB queries
    query_mock = AsyncMock()
    query_mock.get.return_value = [doc_mock_1, doc_mock_2]
    
    collection_mock = MagicMock()
    collection_mock.where.return_value.limit.return_value = query_mock
    ...
    async def side_effect_meta(to_phone, template_name, components, language_code):
    ...
    with patch('app.routers.admin.firestore.AsyncClient', return_value=mock_db), \
        ...
        assert response.errors[0]["phone"] == "3001234567"
