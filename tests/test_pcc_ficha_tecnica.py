...
async def test_habeas_data_gate_before_credit_score():
    ...
    with patch.object(cerebro, '_call_gemini_with_retry_async', new=mock_call_no_consent), \
        ...
        assert re.search(r"\$580,336", response) is not None, "El formato de cuota formateada no cumple con la regla de negocio ($580,336)."
        ...
        assert "Si te interesa a crédito con la inicial de $996,900, las cuotas a 24 meses serían aproximadamente de $580,336 (incluye SOAT y Matrícula). *Nota: Este es un valor aproximado.*" in response, "El resultado debe contener la cadena esperada."
...
async def test_habeas_bypass_interrupt_e2e():
    ...
    with patch.object(cerebro, '_call_gemini_with_retry_async', new=mock_gemini_call), \
        ...
        assert "Si te interesa a crédito con la inicial de $996,900, las cuotas a 24 meses serían aproximadamente de $580,336 (incluye SOAT y Matrícula). *Nota: Este es un valor aproximado.*" in response, (
