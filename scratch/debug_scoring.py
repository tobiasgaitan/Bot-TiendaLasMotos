...
# Mock google.cloud.firestore and firebase_admin
from unittest.mock import MagicMock
import sys
...
class TestCatalogScoring(unittest.TestCase):
    ...
    def test_intent_bonus_trabajo(self):
        ...
        print(f"Results for 'trabajo': {[r['name'] for r in results]}")
    ...
    def test_identity_preservation_raider(self):
        ...
        # We need to simulate a query that has BOTH Raider (Identity) and trabajo (Intent)
        # to see if Identity is preserved.
        results = self.service.search_items("Raider para trabajo")
        print(f"Results for 'Raider para trabajo': {[r['name'] for r in results]}")
        self.assertTrue(len(results) > 0)
        self.assertEqual(results[0]["name"], "TVS Raider 125")

if __name__ == '__main__':
    # Mock config_service to avoid firestore errors
    with patch('app.services.config_service.config_service') as mock_config:
        mock_config.get_registration_cost.return_value = 0
        unittest.main()
